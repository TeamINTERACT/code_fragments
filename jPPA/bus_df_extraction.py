...
Description: Process all bus GPS data and put it in a dataframe.
"""
...
def bus_df(directory, res):
    """
    Create processed dataframe from city bus routes
    :param directory: path to directory containing the stop_times, stops, trips text files
    :param res: space pixel resolution
    :return: processed dataframe from city bus routes
    """
    ...
    df_stop_times[['stop_id']] = df_stop_times[['stop_id']].apply(pd.to_numeric,errors='coerce')

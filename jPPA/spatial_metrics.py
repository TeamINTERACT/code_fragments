...
def get_grid_MODIFIED(x, y, grid_size):
    '''
    Stratifies the location points into a grid of cell size m
    :param x: column with the east-west location values. ALREADY ROUNDED TO GRID RESOLUTION
    :param y: column with the south-north location values. ALREADY ROUNDED TO GRID RESOLUTION
    :param size: integer representing the cell size in meters
    :return: Series with the grid values
    '''
    start_x = min(x)
    ...
    cell_x = ((x - start_x) / grid_size).astype(int).astype(str)
    cell_y = ((y - start_y) / grid_size).astype(int).astype(str)
...
def movement_detection(df, n):
    ...
    def rename_columns():
        ...
        df.columns.values[0] = 'interact_id'
        df.columns.values[1] = 'start_time'
        df.columns.values[2] = 'end_time'
        df.columns.values[3] = 'grid_cell'
        df.columns.values[4] = 'duration'

    # df = df.sort_values(["utc_date", "interact_id"])

    df['duration'] = ((df['grid_cell'] != df['grid_cell'].shift()) | (df['interact_id'] != df['interact_id'].shift())).cumsum()
    df = df.groupby(df['duration'], as_index=False).agg(OrderedDict([('interact_id', 'first'), ('utc_date', ['first', 'last']),
                                                                     ('grid_cell', 'first'), ('duration', 'count')]))
    rename_columns()

    df['trip_length'] = df['duration']
    df['trip_length'] = ((df['duration'] > n) | (df['duration'].shift() > n)).cumsum()
    df = df.groupby(df['trip_length'], as_index=False).agg(OrderedDict([('interact_id', 'first'), ('start_time', 'first'),
